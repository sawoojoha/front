...
//console.log(selectedVideo)
  const Edit = async () => {
    ...
    const data = await fetch(`http://localhost:8123/api/v1/video?userId=${user.id}`);
  ...
  const patchImage = async (e) => {
    try {
      ...
      await fetch(`http://localhost:8123/api/v1/video/${selectedVideo.videoId}/newThumbnail`, {
        body: formData,
        method: "PATCH"
      });
  
      // 썸네일 업데이트 후 선택된 비디오 정보 다시 가져오기
      const response = await fetch(`http://localhost:8123/api/v1/video/${selectedVideo.videoId}`);
